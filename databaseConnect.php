...
if(!$_ENV["VCAP_SERVICES"]){ //local dev
    ...
    if($vcap_services->{'user-provided'}){
        $db = $vcap_services->{'user-provided'}[0]->credentials;
    }
    else if($vcap_services->{'mysql-5.5'}){ //if "mysql" db service is bound to this application
    ...
    else if($vcap_services->{'sqldb'}){ //if ibm-sql-db service is bound to this application
...
//echo "Sucessfully Connected to MySQL DB " . $mysql_database . " with credentials: (" . 
//			$mysql_username . ", " . 
//			$mysql_password . ", " . 
//			$mysql_server_name . ").";
